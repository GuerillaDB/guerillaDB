...
from ..utils.exceptions import ConfigurationError # Assuming you have this custom exception
...
class SQLServerHandler(DBHandlerInterface):
    ...
    def __init__(self,
                 server: str,
                 database: str,
                 **kwargs: Any): # To catch any other params passed by the factory
        """
    ...
    def get_connection(self) -> pyodbc.Connection: # Be specific with pyodbc.Connection
        ...
        """
        try:
            # Ensure connection_string is valid
            ...
            # Log or handle specific pyodbc errors if needed
    ...
    # Using the second (last) definition from your interface.py for save_metadata_to_db
    def save_metadata_to_db(self, manifest: Dict, data_name: str, description: str = "") -> None:
        ...
        try:
            ...
            # First, delete any existing metadata for this data_name
    ...
    def save_compressed_data_to_db(self, data_dict: Dict, data_name: str, chunk_id: int,
        ...
        try:
            # Serialize the data dictionary to bytes
    ...
    def load_db_in_chunks(self,
        ...
        try:
            ...
            while True:
                ...
                # Parameters sequence must match the '?' order: (offset, chunksize)
                params = (offset, chunk_size) # Order matches OFFSET ?, FETCH NEXT ?
                logging.debug(f"Executing query: {query} with params {params}")

                # Combine user options, ensuring 'params' is set correctly for read_sql_query
                read_opts = db_read_options.copy()
                read_opts['params'] = params # Overwrite any user-provided params

                current_chunk = pd.read_sql_query(
                    sql=query,
                    con=cnxn, # Pass the pyodbc connection
                    **read_opts
                )
