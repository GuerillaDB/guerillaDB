...
def calculate_volatility_stats_agg(
    ...
    if df_overlap is not None:
        ...
        if not df_overlap.empty and not df.empty and df_overlap.index[-1] > df.index[0]:  # consider >= instead of >
    ...
    for col in column_names:
        ...
        price_series = combined_df[col].replace(0, np.nan)
...
# Define event dates
INDEX_INCLUSION_DATE = pd.Timestamp('2025-02-02')
INDEX_REMOVAL_DATE = pd.Timestamp('2022-07-15')
RATING_DOWNGRADE_DATE = pd.Timestamp('2025-03-15')
...
def calculate_price_jump_skew(
    ...
    if df_overlap is not None:
        ...
        if not df_overlap.index.max() <= df.index.min():  #consider < instead od <=
